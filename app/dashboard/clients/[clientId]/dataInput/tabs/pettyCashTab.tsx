...
export default function PettyCashTab({ clientId }: PettyCashTabProps) {
  const [areas, setAreas] = useState<{ id: string; name: string }[]>([]);
  const [activeArea, setActiveArea] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  const loadAreas = useCallback(async () => {
    const ref = collection(db, "clients", clientId, "pettyCashAreas");
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold mb-4">Petty Cash by Area</h2>
      ...
      {activeArea ? (
        <AreaItemsTable
          clientId={clientId}
          areaId={activeArea}
          basePath="pettyCashAreas"
        />
      ) : (
        <p className="text-gray-500">Select an area to view petty cash items.</p>
      )}
    </div>
  );
}
